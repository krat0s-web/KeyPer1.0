# Documentation du Code Météo - KeyPer

Ce fichier contient toutes les parties de code liées à la fonctionnalité météo de votre projet KeyPer.
Vous pouvez copier ces sections dans votre projet original pour intégrer la fonctionnalité météo.

---

## 📋 TABLE DES MATIÈRES

1. [Modèle - Champ villes_favorites_meteo](#1-modèle---champ-villes_favorites_meteo)
...
6. [Views - Fonction dashboard (partie météo)](#6-views---fonction-dashboard-partie-météo)
...
11. [URLs - Routes API météo](#11-urls---routes-api-météo)
12. [Template - HTML météo dans dashboard.html](#12-template---html-météo-dans-dashboardhtml)
13. [Template - JavaScript météo dans dashboard.html](#13-template---javascript-météo-dans-dashboardhtml)

---

## 1. MODÈLE - Champ villes_favorites_meteo

**Fichier :** `maison_app/models.py`

**Localisation :** Dans la classe `Utilisateur`, après le champ `photo_profil`
...
**Note :** Ce champ stocke une liste de noms de villes favorites pour la météo.
...
**Action :** Créer ce fichier de migration et l'appliquer avec `python manage.py migrate`
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('maison_app', '0025_inventaire_quantite_alerte_min'),  # ⚠️ ADAPTER selon votre dernière migration
    ]
...
**Note :** Adaptez le `dependencies` selon votre dernière migration existante.
...
**Localisation :** À la fin du fichier, dans la section des configurations API
...
**Note :** Vous pouvez aussi utiliser une variable d'environnement pour la clé API.
...
**Action :** Si ce fichier n'existe pas, créez-le. Si il existe, ajoutez la fonction.
...
def get_weather_data(city_name):
    ...
    """
    # Récupérer la clé et l'URL définies dans settings.py
    ...
    try:
        ...
        # Erreur si la structure de la réponse JSON n'est pas celle attendue (ville non trouvée, etc.)
        print("Erreur: Structure de données météo invalide.")
        return None
```

**Note :** Cette fonction nécessite le package `requests` (installer avec `pip install requests`).
...
## 6. VIEWS - Fonction dashboard (partie météo)
...
```python
    ...
    # Vérifie si l'utilisateur a des villes favorites et que la liste n'est pas vide
    if request.user.villes_favorites_meteo and len(request.user.villes_favorites_meteo) > 0:
        try:
            # Récupère la première ville favorite
            first_favorite_city = request.user.villes_favorites_meteo[0]
            
            # Récupère les données météo via l'API client
            ...
            if weather_data:
                ...
                # Ajoute les données de recommandation au dictionnaire
            ...
            # En cas d'erreur (API, liste mal formatée, etc.)
            print(f"Erreur lors du chargement de la météo initiale: {e}")
            initial_weather = None
    
    return render(request, 'maison_app/dashboard.html', {
        # ... vos autres contextes existants ...
        'initial_weather': initial_weather,  # ⚠️ AJOUTER cette ligne dans votre contexte
    })
```
...
**Localisation :** À la fin du fichier, avec les autres vues API (lignes ~2707-2734)

```python
@login_required
def api_get_weather(request):
    """
    Récupère les données météo pour une ville demandée par l'utilisateur (via AJAX)
    et ajoute une recommandation vestimentaire basée sur la température ET l'état du ciel.
    """
    ...
    if meteo_data:
        # Récupérer les données nécessaires pour la recommandation
        temp = meteo_data.get('temp')
        weather_main = meteo_data.get('main')  # Récupération de l'état principal (Rain, Clear, etc.)

        # Appel de la fonction avec la température ET l'état principal
        reco = get_clothing_recommendation(temp, weather_main) 
        
        # Ajouter les données de recommandation au JSON de réponse
        ...
        return JsonResponse({'error': 'Ville non trouvée ou erreur API.'}, status=404)
```
...
**Localisation :** Après `api_get_weather` (lignes ~2736-2764)
...
def api_add_favorite_city(request):
    ...
    if request.method == 'POST':
        ...
        if city_name not in favorites:
            ...
            return JsonResponse({'success': True, 'message': f"'{city_name}' ajouté aux favoris."})
        else:
            return JsonResponse({'error': 'Cette ville est déjà dans vos favoris.'}, status=400)
...
**Localisation :** Après `api_add_favorite_city` (lignes ~2767-2790)
...
def api_remove_favorite_city(request):
    ...
    if request.method == 'POST':
        ...
        if city_name in favorites:
            ...
            request.user.villes_favorites_meteo = favorites
            request.user.save()
            return JsonResponse({'success': True, 'message': f"'{city_name}' retiré des favoris."})
        else:
...
**Localisation :** Après `api_remove_favorite_city` (lignes ~2792-2827)

```python
def get_clothing_recommendation(temp, weather_main):
    """Détermine la recommandation vestimentaire en fonction de la température et de l'état du ciel."""
    
    # 1. Condition Spécifique : PLUIE (Prime sur la température)
    if weather_main in ['Rain', 'Drizzle', 'Thunderstorm']:
        return {
            'text': "Attention, il pleut ! Tenue imperméable.",
            'image_tag': 'tenue_pluvieuse',
            'class': 'bg-info text-dark'
        }

    # 2. Condition basée sur la Température (si pas de pluie)
    
    # Tenue d'hiver (Froid)
    if temp <= 10:
        return {
            'text': "Froid - Tenue d'hiver recommandée !",
            'image_tag': 'tenue_hiver', 
            'class': 'bg-primary'
        }
    
    # Tenue normale (Intermédiaire)
    elif 11 <= temp <= 25:
        return {
            'text': "Température agréable - Tenue normale.",
    ...
    # Tenue d'été (Chaud)
        ...
        return {
            'text': "Chaud - Tenue légère (d'été) recommandée.",
            'image_tag': 'tenue_ete',
            'class': 'bg-danger'
        }
```

**Note :** Cette fonction nécessite des images dans `static/images/vetements/` :
- `tenue_pluvieuse.png`
- `tenue_hiver.png`
- `tenue_normale.png`
- `tenue_ete.png`
...
**Note :** Si vous utilisez l'autocomplétion de villes, ajoutez aussi :
```python
...
## 12. TEMPLATE - HTML météo dans dashboard.html

**Fichier :** `maison_app/templates/maison_app/dashboard.html`

**Localisation :** Dans le contenu principal, où vous souhaitez afficher le bloc météo (lignes ~68-159)
    ...
    <div class="col-md-12">
        <div class="card border-0 shadow-lg rounded-4">
            <div class="card-body p-4">
                <h5 class="mb-3"><i class="bi bi-cloud-sun me-2"></i> Consulter la Météo</h5>
                ...
                <div class="input-group">
                    <input type="text" 
                           id="city-search"
                           class="form-control"
                           placeholder="Rechercher une ville pour la météo..."
                           aria-label="Rechercher une ville"
                           required>
                    <button type="button" class="btn btn-primary" id="load-weather-btn">
                        <i class="bi bi-search me-1"></i> Afficher Météo
                    </button>
                </div>
                
                {# Zone pour les suggestions d'autocomplétion #}
                ...
                <div class="mt-4">
                    ...
                    {# Conteneur des résultats MÉTÉO (avec pré-chargement) #}
                    <div id="weather-results" class="mt-3">
                        ...
                        <div class="card border-0 shadow-lg rounded-4 mb-4" style="background: linear-gradient(45deg, #0d6efd, #0dcaf0); color: white;">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h4 class="mb-0"><i class="bi bi-cloud-sun me-2"></i> Météo à {{ initial_weather.city }}</h4>
                        ...
                        <div class="alert alert-info text-center" role="alert">
                            {% if user.villes_favorites_meteo %}
                            Impossible de charger la météo pour la première ville favorite. Veuillez réessayer.
                            {% else %}
                ...
                {# --- FIN BLOC MÉTÉO FAVORIS & RÉSULTATS --- #}

            </div>
        </div>
    </div>
</div>
{# --- FIN BLOC MÉTÉO DYNAMIQUE --- #}
```
...
## 13. TEMPLATE - JavaScript météo dans dashboard.html

**Fichier :** `maison_app/templates/maison_app/dashboard.html`

**Localisation :** Dans un bloc `<script>` à la fin du template, avant `{% endblock %}` (lignes ~440-646)
...
$(document).ready(function() {
    ...
    // 💡 Définition de l'URL Statique de base pour les images de vêtements
    ...
    const handleFavoriteAction = function(city, actionUrl, verb) {
        $.ajax({
            ...
            success: function(response) {
                // Actualiser la liste de favoris (méthode simple : recharger la page)
    ...
    // --- GESTION DE L'AUTOCOMPLÉTION (GeoNames API) ---
    // ⚠️ OPTIONNEL : Si vous n'utilisez pas l'autocomplétion, vous pouvez supprimer cette section
    ...
    // --- GESTION DE L'AFFICHAGE DE LA MÉTÉO (OpenWeatherMap API) ---
    
    const loadWeather = function(cityToLoad) {
        const city = cityToLoad || searchInput.val().trim();
        if (!city) return;

        weatherResults.html('<div class="alert alert-warning text-center">Chargement des données météo...</div>');
        loadWeatherBtn.prop('disabled', true);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // Récupérer les données de recommandation
                ...
                <div class="card border-0 shadow-lg rounded-4 mb-4" style="background: linear-gradient(45deg, #0d6efd, #0dcaf0); color: white;">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h4 class="mb-0"><i class="bi bi-cloud-sun me-2"></i> Météo à ${data.city}</h4>
                            ${favoriteButton}
                        </div>
                        <div class="d-flex align-items-center mb-4">
                        ...
                        {# BLOC RECOMMANDATION VESTIMENTAIRE (GÉNÉRÉ DYNAMIQUE) #}
            ...
            error: function(xhr) {
                const errorMessage = xhr.responseJSON && xhr.responseJSON.error 
                                     ...
                                     : "Impossible de récupérer les données météo. Veuillez vérifier le nom de la ville.";
    ...
    // Chargement de la météo via les tags de favoris existants (pour les favoris affichés par Django)
    ...
    // Événement spécifique pour le bouton de retrait pré-rendu (météo initiale)
    // Nous devons utiliser un gestionnaire d'événements délégué car ce bouton est remplacé par AJAX
    ...
    // Permettre de charger la météo en appuyant sur Entrée dans le champ de recherche
    ...
    // 💡 Déchargement initial du bouton de météo si le champ de recherche est vide
...
**Note :** Ce code nécessite jQuery. Assurez-vous qu'il est chargé dans votre template.
...
### Images nécessaires
Créez un dossier `static/images/vetements/` et ajoutez les images suivantes :
- `tenue_pluvieuse.png`
- `tenue_hiver.png`
- `tenue_normale.png`
- `tenue_ete.png`
...
Si vous souhaitez utiliser l'autocomplétion de villes, vous aurez besoin de :
...
3. Créer la vue `api_search_city` dans `views.py`
...
- [ ] Ajout du champ `villes_favorites_meteo` dans le modèle `Utilisateur`
- [ ] Création et application de la migration
- [ ] Configuration OpenWeatherMap dans `settings.py`
- [ ] Création/ajout de `get_weather_data` dans `api_clients.py`
- [ ] Import de `get_weather_data` dans `views.py`
- [ ] Ajout du code météo dans la fonction `dashboard`
- [ ] Création de `api_get_weather`
- [ ] Création de `api_add_favorite_city`
- [ ] Création de `api_remove_favorite_city`
- [ ] Création de `get_clothing_recommendation`
- [ ] Ajout des routes dans `urls.py`
- [ ] Ajout du HTML météo dans `dashboard.html`
- [ ] Ajout du JavaScript météo dans `dashboard.html`
- [ ] Création des images de vêtements dans `static/images/vetements/`
- [ ] Installation de `requests` si nécessaire
